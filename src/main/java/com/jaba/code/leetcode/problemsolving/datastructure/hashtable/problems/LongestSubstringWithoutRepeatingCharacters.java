...
public class LongestSubstringWithoutRepeatingCharacters {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        new LongestSubstringWithoutRepeatingCharacters().lengthOfLongestSubstring("au"));
    System.out.println(
    ...
    System.out.println(
        new LongestSubstringWithoutRepeatingCharacters().lengthOfLongestSubstring(" "));
    System.out.println(
        new LongestSubstringWithoutRepeatingCharacters().lengthOfLongestSubstring("a"));
    System.out.println(
        new LongestSubstringWithoutRepeatingCharacters().lengthOfLongestSubstring(null));
    System.out.println(
  ...
  public int lengthOfLongestSubstring(String s) {
    ...
    while (index < s.length()) {
      int indexNewChar =
          index + 1 < s.length()
              ? s.substring(iniSubstring, index + 1).indexOf(s.charAt(index + 1))
              : Integer.MAX_VALUE;
      if (indexNewChar < 0) {
        index++;
      } else {
